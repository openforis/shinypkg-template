...
The shiny app works with a number of assets, i.e. images, CSS style, javascript handlers, translation files etc.
...
The solution is to place the files under `inst/assets` in the package directory, which can be made available with:

```{r}
system.file("assets/FILENAME", package = "PKGNAME")
```

To avoid having to point to the package files for each asset, a script `R/zzz.R` can be added with the following code, in order to shorten the paths to `assets/FILENAME`:
...
The assets for the backbone app are:

-   a javascript code to handle changing tabset within a navbar page
...
The assets can be downloaded from the Github repository manually or with this code:
...
**NOTE 1**: This doesn't work for only one case: the translation file called in the main app script. The following code snippet requires the `system.file()` function explicitly in the main app script `R/shiny_run_APPNAME.R`:
